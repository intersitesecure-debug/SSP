...
## 1c. `tests/SSP.Tests` is an `Exe`, and why that needs `GenerateProgramFile=false`

Observed symptom (any SDK, once `tests/SSP.Tests` sets `<OutputType>Exe</OutputType>`):

```
tests\SSP.Tests\Activation\ClockLockProbe.cs(35,25): error CS0017:
...
* `tests/SSP.Tests` is deliberately an **executable**.
  `ClockRollbackStateTests.FileLease_IsExclusiveAcrossProcesses` must observe
...
* `Microsoft.NET.Test.Sdk` *also* contributes an entry point to every C# test
  ...
  `AutoGeneratedProgram.Main`. In a class library that Main is inert; in an
  `Exe` it is a second candidate, and the compiler reports `CS0017` against
  the *other* Main — the probe — which is why the error points at a file that
  is perfectly correct.

Resolution (build configuration only, no code change), in
`tests/SSP.Tests/SSP.Tests.csproj`:

| Property | Effect |
| --- | --- |
| `<GenerateProgramFile>false</GenerateProgramFile>` | The SDK's supported "this test project brings its own `Main`" switch: the auto-generated program is no longer compiled in, so exactly one entry point remains. |
| `<StartupObject>SSP.Tests.Activation.ClockLockProbe</StartupObject>` | Pins that entry point by name, so the build stays unambiguous (and self-documenting) even if another package contributes a `Main` later. |

Neither property affects test discovery or execution: VSTest loads the
assembly through `testhost` and never invokes the entry point. Only the
`dotnet exec` child process started by that one test does.
