...
- **What changed:**
  ...
  - The optional `ILicenseTimeStateLock` seam leaves `ILicenseStateStore` unchanged. SSP's reentrant `.license-state.dat.lock` lease serializes the complete read/sample/merge/save/readback across store instances/processes; its thirty-second acquisition bound uses elapsed time, not UTC, and is sized so legitimate contention (a burst of concurrent checkpoints, each holding the lease for one read/sample/merge/save/readback transaction) serializes and succeeds — only a holder that never releases within the bound fails closed. The empty lock file is never deleted. Replacement stores without this seam are synchronized only when callers share the same instance.
