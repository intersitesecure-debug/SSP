...
**Phase 6 validation note (2026-09-06):** the clock-rollback implementation and
its automated test source are present, but restore, compilation and the full
suite could not execute here (`dotnet` is unavailable). T36 test references are
coverage to execute, **not** passing-test claims or a completed sign-off. The
user-supplied merged baseline is 697/697; it was not reproduced in this session.
See `Security Correction.md`, Step 8, for the exact attempted commands.
...
fails the service start closed. Protected local UTC history and its checks at
validation, activation and new-operation authorization are in scope through T36
(roadmap Phase 6, M-6). This is not a trusted-time service: it reuses the license
state and witness, without network access, TPM assumptions or new keys.
Licensing never changes the wire protocol or the data plane; the client carries
no licensing code.
...
| Anti-rollback floor (`.license-state.dat`) | Same directory, on `ProtectedFileStore.ProtectedFileNames` | DPAPI LocalMachine envelope on Windows (AES-GCM fallback elsewhere); also records `ActivatedLicenseId`; can only *restrict* authorization, never grant it; corruption ⇒ `state_store_unavailable` ⇒ fail closed. Phase 4 (M-3): the record is additionally **bound to the installation** (domain-separated hashed MachineGuid) and stamped with a **monotonic state epoch**; a record naming another installation fails closed; Phase 6 adds `ClockStateVersion` and `LastObservedUtc` (restrictive local UTC history, including observed expiry), validated before use; legacy `LastValidatedUtc` seeds its lower bound |
| Anti-rollback **witness** (`.ssp-state-witness/license/{sha256(licensing-dir)}/.witness.dat`) | One directory level ABOVE the licensing directory — outside it, so deleting/restoring the licensing directory cannot take the witness with it | Redundant envelope-encrypted installation binding, state epoch, highest accepted sequence, accepted/activated IDs (T33) and versioned local UTC checkpoint (T36). Primary deletion recovers the retained values; older primary epoch, corrupt/plaintext/foreign witness or invalid clock metadata fails closed. An intact primary may repair a missing witness, but Phase 6 requires **both** protected writes before licensing authorization. Primary-first ordering and max-merge are retained; legacy sequence-only and enrollment witness behavior is unchanged |
...
| T3 | Wrong product / wrong installation / expiry / not-yet-valid / revoked artifact | Product id bound at composition (`SspLicensing.ProductId`); purpose-bound installation hash; both signed validity windows use one `IClock` sample checked against protected local UTC history (T36) | `LicensingFailClosedMatrixTests`; `SspLicensingAndTrustAnchorTests`; `InstallationBindingTests`; `ClockRollbackEnforcementTests.EitherSignedValidityWindow_ExpiresIndependently_AndCannotBeRevived` |
...
| T11 | Expiry/revocation invisible after start | One revalidation timer (`SspActivationService`, 30 min default) continues full provider/signature/revocation checks. Phase 6 additionally checks clock integrity and both validity windows on each authorization of the already authenticated current license; expiry/rollback denial does not wait for that timer | `LicensingCompositionTests` timer suite; `ClockRollbackEnforcementTests.EveryRuntimeGate_DeniesRollbackWithoutWaitingForRefresh`; `TimerObservedExpiry_CannotBeReversed_AndOnlyAValidRenewalRecovers` |
...
| T36 | **Roll back UTC to revive an expired payload or key certification (Phase 6 / M-6), or keep authorizing from cached Valid after time-integrity failure** | `LicenseTimeIntegrity` compares a post-signature UTC sample with the maximum retained local time (primary, witness, legacy successful-validation timestamp and running guard). Strict regression denies; equality is allowed. Both signed windows use the same sample and expiration observations are checkpointed without acceptance/activation. Required primary/witness writes and readback precede Valid publication and new-operation decisions. A reentrant cross-process file lease covers read/sample/merge/write/readback, preserving concurrent acceptance/activation. Invalid/missing initialized metadata, clock exceptions and checkpoint/lease/write failures deny; safe Warning events 4616/4617 identify rollback or unavailable time integrity. Valid legacy records migrate in place; primary deletion recovers witnessed time. This is local history, not authoritative UTC; first use, frozen/unobserved time and coordinated loss of all history remain limitations (§7.1/§9) | `ClockRollbackTests` (forward/equal, single sample, expiry/restart, migration, failures, stale Apply, safe events); `ClockRollbackStateTests` (protected copies, repair, actual write faults, distinct-instance and cross-process leases, delayed writer vs renewal/activation); `ClockRollbackEnforcementTests` (all runtime gates, both windows, activation, startup/timer, real handshake and enrollment); `SspSecurityEventSinkTaxonomyTests`. **Execution pending — Step 8** |
...
7. The system clock is host-controlled. Phase 6 compares UTC against protected
   **previously observed local time**, not an authority-certified clock. The
   sequence floor alone did not prevent same-sequence expiration replay; T36
   adds an independent time floor and checks both signed validity windows.
   Offline local history still cannot authenticate initial, frozen or
   unobserved time, or survive coordinated loss/rollback of all its copies.
...
### 7.1 Phase 6 clock-state contract, migration and recovery

* **Lower bound, never a grant.** `LastObservedUtc` is the greatest retained
  licensing UTC observation, not a server-supplied timestamp. Only after the
  relevant signatures, status and bindings pass does the validator observe
  time. `now < floor` denies; `now == floor` is allowed. UTC-equivalent offset
  or DST representations do not regress. No tolerance, expiry grace or file
  modification-time heuristic is used. `NotBefore` is inclusive, `ExpiresAt`
  exclusive for both the payload and its certification.
* **Remember restrictive observations too.** An authenticated artifact seen
  expired or not yet valid updates time only: no sequence acceptance or code
  activation. `LastValidatedUtc` still records successful acceptance and is
  refreshed for the same license. The running guard retains higher loaded and
  observed time even after a rejected observation or failed checkpoint.
* **Compatible bootstrap.** Version 0 with no `LastObservedUtc` is legacy;
  `LastValidatedUtc`, when present, supplies the migration lower bound. Valid
  legacy encrypted or plaintext primary records retain their binding, sequence
  and activation on upgrade. A legacy record/witness with no time cannot prove
  historical UTC; the first authenticated observation initializes version 1.
  Unknown versions, version 1 without time, version 0 with a new timestamp,
  malformed timestamps or initialized plaintext state fail closed instead of
  being treated as fresh. A save cannot erase initialized evidence by supplying
  a legacy record; existing corrupt/foreign history is not overwritten to
  reset the clock.
* **Persistence is required.** Every new licensing authorization of a current
  Valid artifact checks time/windows and completes the protected checkpoint.
  A policy is evaluated only while the manager is Valid: replacing a denial's
  diagnostic reason with a later failed/missing reload cannot clear lockdown.
  The native store max-merges primary/witness time and requires both writes.
  Missing primary recovers witnessed time; missing witness must be repaired
  from an intact primary before authorizing. Clock read failure uses
  `time_integrity_unavailable`; state/lock/persistence/readback failures retain
  `state_store_unavailable`; both emit `TimeIntegrityUnavailable` (4617).
  Regression emits `ClockRollbackDetected` (4616). Both are Warning, without
  credentials, raw artifacts, code hashes, signature material or exception
  messages. Reporting failure does not alter the security decision.
...
* **Recovery never resets history.** Inspect the licensing reason/security
  ...
  UTC to at least the retained floor, then run the existing full revalidation
  or load a currently valid signed renewal. Renewal alone cannot clear a
  rolled-back clock; clock correction cannot un-expire a signed artifact.
  Do not delete witnesses, lower timestamps/versions or weaken the signed
  windows. A legitimate backward correction or accidental far-future jump can
  cause persistent denial until UTC reaches the retained lower bound.
* **Availability and limits.** New admissions now require local I/O and the
  lease; read/write failure is not a reason to trust cached Valid. Checks are
  at the existing admission boundaries, not per packet, and already-admitted
  tunnels are not forcibly disconnected. The timer still performs full
  cryptographic/provider/revocation validation. Phase 2 enrollment cooldown
  arithmetic and Phase 4 enrollment witness behavior are unchanged; the
  licensing gate runs before enrollment code generation/consumption.
...
* **Artifact size is bounded** (256 KiB cap, fail-closed) to blunt resource
  exhaustion.
* **Clock manipulation** is partially mitigated by protected local time
  history (T36, §7.1), not by trusted UTC. Initial/no history, expiration never
  observed and checkpointed, frozen/nonregressing manipulated time, complete
  snapshot/deletion of history, and privileged process/OS tampering remain
  limitations. A timestamp that reaches neither protected copy before
  process/power loss cannot be reconstructed offline; another/fresh guard also
  has no durable evidence of an observation retained only by a failing guard.
* **Enrollment attempt tracking is local server state, witnessed outside the
  ...
  can still shift the *witnessed* cooldown: Phase 6 gates observed licensing
  rollback before code generation, but does not replace Phase 2's separate
  clock arithmetic (forward manipulation remains possible). Clock changes
  cannot restore attempts after the OTT has been revoked, and three guesses
  against a 10-digit code remain infeasible.
...
| Risk | Why accepted |
| --- | --- |
| Coordinated rollback of BOTH the protected state file AND its witness (or full machine/product-root state reconstruction) by a local administrator can reset the sequence/time floors and enrollment memory after process loss (T33/T34/T36) | Phase 4 closes the cheap single-artifact attacks (file deletion alone; file rollback alone; directory restore alone — the witness lives outside the protected directory). Defeating it now requires finding, rolling back or destroying two independently stored artifacts in different directory trees; fully eliminating the class needs tamper-resistant storage (TPM) and/or trusted time / online status, all incompatible with the offline architecture (reference ARCHITECTURE §11: the worst case is re-enabling an older, previously legitimately accepted license — never an unsigned one) |
| Persistently failing **enrollment / legacy sequence-only** witness writes can disable their deletion detection (T33/T34) | The earlier config/primary-first best-effort behavior is retained for those records (`Enrollment.StateWitnessWriteFailed`). It is **not** the Phase 6 licensing policy: a time checkpoint whose primary or witness cannot be written denies authorization, even if the other write succeeds |
| A crash/power loss between primary and witness writes can leave a lagging witness; the two files are not one power-loss-atomic transaction (T33/T34/T36) | Primary-first ordering and existing protected-file primitives are retained. Phase 6 does not authorize until both writes return and readback verifies the checkpoint; a partial write denies the current call and any surviving higher evidence restricts future validation. Lost, never-durable observations cannot be recovered after process loss, and coordinated rollback/removal of all surviving evidence remains outside the offline guarantee |
...
| Host time manipulation outside retained observations (T36) | Observed/checkpointed expiration and backward UTC below either retained copy now deny. A first-use clock with no history, unobserved expiration, frozen/nonregressing manipulated time or a complete coordinated snapshot can still evade local-history detection. Trusted absolute time/tamper-resistant storage is not available in this offline architecture |
| Legitimate backward correction, accidental large forward jump, or unavailable checkpoint storage denies new admissions (T36) | Strict monotonicity and mandatory protected writes are intentional fail-closed availability costs. There is no timestamp-reset escape hatch: restore storage availability and nonregressing UTC, then fully validate a signed artifact still inside both windows. Local per-admission file I/O and lease latency must be included in deployment/load validation |
...
| Local clock history reviewed (Phase 6 / M-6) | T36 added: versioned monotonic UTC primary/witness checkpoint, compatible legacy migration, strict rollback/window enforcement at validation/activation/admission, mandatory writes/readback and serialized RMW, safe Warning IDs 4616/4617; §7.1 records recovery and offline limits. Static self-review on `arena/01a07576-ssp` (2026-09-06). New unit/native/runtime test source is present; **build and full-suite execution blocked** by missing .NET SDK, so Phase 6 is not signed off (roadmap Step 8) |
